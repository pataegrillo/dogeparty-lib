...
from dogepartylib.lib import check
...
from dogepartylib.lib import arc4
from dogepartylib.lib.transaction_helper import p2sh_encoding

from .messages import (send, order, dogepay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve, destroy, sweep, dispenser)
...
from .exceptions import DecodeError, DOGEOnlyError
...
TABLES = ['credits', 'debits', 'messages'] + \
         ...
         'bet_expirations', 'bets', 'broadcasts', 'dogepays', 'burns',
...
def parse_tx(db, tx):
    ...
    try:
        with db:
            ...
            elif message_type_id == dogepay.ID:
                dogepay.parse(db, tx, message)
...
def initialise(db):
    ...
    cursor.execute('''SELECT * FROM assets WHERE asset_name = ?''', (config.DOGE,))
    if not list(cursor):
        cursor.execute('''INSERT INTO assets VALUES (?,?,?,?)''', ('0', config.DOGE, None, None))
        cursor.execute('''INSERT INTO assets VALUES (?,?,?,?)''', ('1', config.XDP, None, None))
...
def get_tx_info(tx_hex, block_parser=None, block_index=None, db=None):
    """Get the transaction info. Returns normalized None data for DecodeError and DOGEOnlyError."""
    try:
        return _get_tx_info(tx_hex, block_parser, block_index)
    except DecodeError as e:
        return b'', None, None, None, None, None
    except DOGEOnlyError as e:
...
def _get_swap_tx(decoded_tx, block_parser=None, block_index=None, db=None):
    ...
    for vout in decoded_tx.vout:
        ...
        if db != None and dispenser.is_dispensable(db, destination, doge_amount):
            check_sources = True
            outputs.append((destination, doge_amount))
...
def get_tx_info1(tx_hex, block_index, block_parser=None):
    ...
    destination, doge_amount, data = None, None, b''
    ...
    for vout in ctx.vout:
        ...
        if not destination and not doge_amount and not data:
            address = get_address(vout.scriptPubKey)
            if address:
                destination = address
                doge_amount = vout.nValue
    ...
    return source, destination, doge_amount, fee, data, None
...
def get_tx_info2(tx_hex, block_parser=None, p2sh_support=False, p2sh_is_segwit=False):
    ...
    destinations, doge_amount, fee, data = [], 0, 0, b''
    ...
    return sources, destinations, doge_amount, round(fee), data, None
...
def kickstart(db, dogecoind_dir):
    if dogecoind_dir is None:
        if platform.system() == 'Darwin':
            dogecoind_dir = os.path.expanduser('~/Library/Application Support/Dogecoin/')
        elif platform.system() == 'Windows':
            dogecoind_dir = os.path.join(os.environ['APPDATA'], 'Dogecoin')
        else:
            dogecoind_dir = os.path.expanduser('~/.dogecoin')
    if not os.path.isdir(dogecoind_dir):
        raise Exception('Dogecoin Core data directory not found at {}. Use --dogecoind-dir parameter.'.format(dogecoind_dir))

    cursor = db.cursor()

    logger.warning('''Warning:
- Ensure that dogecoind is stopped.
- You must reindex dogecoind after the initialization is complete (restart with `-reindex=1`)
- The initialization may take a while.''')
    ...
    chain_parser = ChainstateParser(os.path.join(dogecoind_dir, 'chainstate'))
    last_hash = chain_parser.get_last_block_hash()
    chain_parser.close()

    # Start block parser.
    block_parser = BlockchainParser(os.path.join(dogecoind_dir, 'blocks'), os.path.join(dogecoind_dir, 'blocks/index'))
            ...
            if len(transactions):
                ...
                for tx_chunk in tx_chunks:
                    sql = '''INSERT INTO transactions
                                (tx_index, tx_hash, block_index, block_hash, block_time, source, destination, doge_amount, fee, data)
                             VALUES '''
                    bindings = ()
                    bindings_place = []
...
def follow(db):
    ...
    while True:
        ...
        if block_index <= block_count:
            ...
            block = backend.getjsonblock(block_hash)
            #previous_block_hash = bitcoinlib.core.b2lx(bytearray.fromhex(block["previousblockhash"]))
            ...
            # For each transaction in Dogecoin Core mempool, if it’s new, create
                ...
                if tx_hash in old_mempool_hashes:
                    for message in old_mempool:
                        if message['tx_hash'] == tx_hash:
                            xdp_mempool.append((tx_hash, message))

                # If not a supported XDP transaction, skip.
            ...
            #  - is txindex enabled in Dogecoind?
            ...
            logger.getChild('mempool').debug('Refresh mempool: %s XDP txs seen, out of %s total entries (took %ss, next refresh in %ss)' % (
                len(xdp_mempool), len(raw_mempool),
